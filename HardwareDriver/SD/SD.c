...
{	  
	...
	SDIO_DataInitStructure.SDIO_DataBlockSize= power<<4 ;//清除DPSM状态机配置 DBBLOCKSIZE占据4-7位，故左移4位  lxd 18.7.21
	...
	{
		SD_level = rt_hw_interrupt_disable();			//屏蔽中断，保存中断状态
		...
		rt_hw_interrupt_enable(SD_level);	//恢复中断状态
...
SD_Error SD_ReadMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
	...
	{									    
		...
		{
			SD_level = rt_hw_interrupt_disable();		//屏蔽中断 保存之前中断状态
